...
parameters {
  real<lower=0,upper=20> epsilon[4]; // error to the latent process
  simplex[4] theta; //weights
  vector[T] l[4]; // latent variables
  simplex[5] cht;
}
...
model {
  ...
  vector[4] lp; // for iterating log probability
...
generated quantities{  
   ...
   for (k in 1:K) {
    ...
    logModel = log_sum_exp(log_theta2)+dirichlet_lpdf(theta | [1,1,1,1]');
   }
    for (h in 1:4)
