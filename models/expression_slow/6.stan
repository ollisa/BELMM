...
parameters {
  array[6] real<lower=0, upper=20> epsilon; // error to the latent process
  simplex[6] theta; //weights
  array[6] vector[T] l; // latent variables
  simplex[7] cht;
}
transformed parameters {
  ...
  log_theta = log(theta);
  l_init = a + head(cumulative_sum(cht), 6) * (b - a); //https://groups.google.com/g/stan-users/c/04GSu-ql3vM
}
model {
  vector[6] lp; // for iterating log probability
  
  target += dirichlet_lpdf(theta | [1,1,1,1,1,1]');
...
generated quantities {
  real logModel;
  vector[6] log_theta2 = log_theta;
  
  for (k in 1 : K) {
    for (h in 1 : 6) 
      log_theta2[h] += normal_lpdf(z[ k , :] | l[h,  : ], epsilon[h]);
    
    logModel = log_sum_exp(log_theta2) + dirichlet_lpdf(theta | [1,1,1,1,1,1]');
  }
  for(h in 1:6)
