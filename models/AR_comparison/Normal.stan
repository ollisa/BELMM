...
model {
  ...
  for (k in 1 : K) {
    ...
    for (h in 1 : H) {
      lp[h] = lp[h]
              + normal_lpdf(z[2 : T, k] | phi0[h]
                                          + phi1[h] * z[1 : (T - 1), k], sigma[h]);
    }
...
generated quantities {
  ...
  for (k in 1 : K) {
    for (h in 1 : H) {
      log_theta[h] += normal_lpdf(z[2 : T, k] | phi0[h]
                                                + phi1[h] * z[1 : (T - 1), k], sigma[h]);
    }
  }
  logModel = log_sum_exp(log_theta);
  for (h in 1 : H) {
    logModel += normal_lpdf(phi0[h] | 0, 40) + normal_lpdf(phi1[h] | 0, 1)
                + inv_gamma_lpdf(sigma[h] | 1, 1);
  }
}
