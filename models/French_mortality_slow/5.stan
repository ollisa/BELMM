...
parameters {
  array[5] real<lower=0, upper=20> epsilon; // error to the latent process
  simplex[5] theta; //weights
  array[5] vector[T] l; // latent variables
  simplex[6] cht;
}
...
model {
  vector[5] lp; // for iterating log probability
  
  target += dirichlet_lpdf(theta | [1,1,1,1,1]');
...
generated quantities {
  ...
  for (k in 1 : K) {
    ...
    logModel = log_sum_exp(log_theta2) + dirichlet_lpdf(theta | [1,1,1,1,1]');
  }
  for(h in 1:5)
